...
const Navbar = () => {
  ...
  const navLinksStyle = { listStyle: 'none', display: windowWidth > 768 ? 'flex' : 'block', gap: '1.5rem', margin: 0, padding: 0 };
  const navLinkItemStyle = { padding: windowWidth > 768 ? '0' : '1rem', textAlign: 'center' };
  const toggleStyle = { display: windowWidth > 768 ? 'none' : 'block', cursor: 'pointer' };
  const barStyle = { height: '3px', width: '25px', backgroundColor: '#fff', margin: '4px 0' };
  const logoutStyle = { ...linkStyle, border: '1px solid #fff', padding: '0.3rem 0.8rem', borderRadius: '4px' };

  return (
    <nav style={navStyle}>
      <div style={brandStyle}>
        <a href="/" style={linkStyle}>Belfast Rides</a>
      </div>
